...
class SaleOrderAjax extends \CBitrixComponent
{
	...
	protected function registerAndLogIn($userProps)
	{
        ...
        $phoneNumber = \PhoneCheckComponent::getPhoneNumber(strval($userProps['PHONE']), '');

        if (!$phoneNumber->isValid()) {
            $this->addError(Loc::getMessage('ORDER_FIELD_PERSONAL_PHONE_VALID'));
            return 0;
        } elseif (!\PhoneCheckComponent::checkConfirmPhoneForParentComponent($phoneNumber, '')) {
            $this->addError(Loc::getMessage('ORDER_FIELD_PERSONAL_PHONE_CONFIRM'));
            return 0;
        }
        ...
        $arUser = UserTable::getList([
            'filter' => ['=PERSONAL_PHONE' => $phoneNumber->format(Format::E164)],
        ])->fetch();

        if ($arUser && ($arUser['ACTIVE'] !== 'Y' || $arUser['BLOCKED'] === 'Y')) {
            $this->addError(Loc::getMessage('ITS_AGENCY_COMPONENT_PHONE_AUTH_ERROR_USER_BANNED'));
            return 0;
        }
	...
	protected function obtainDelivery()
	{
        ...
        $this->arResult['CUSTOM_DELIVERY_CONTENT'] = ob_get_clean();
	}
