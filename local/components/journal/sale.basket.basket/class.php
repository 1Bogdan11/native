...
class CBitrixBasketComponent extends CBitrixComponent
{
	...
	protected function initialLoadAction()
	{
		...
		$this->arResult = array_merge($this->arResult, $basketItems);
    ...
    protected function selectItemWithoutPropsAction()
    {
        $currentId = (int)$this->request->get('basketItemId');
        $offerId = $this->request->get('basketOfferId') ?: [];

        $resultObj = $this->changeProductOffer($currentId, self::SEARCH_OFFER_BY_ID, ['ID' => $offerId], true);

        $result = [];
        $result['DELETE_ORIGINAL'] = 'Y';
        $result['BASKET_DATA'] = $this->getBasketItems();
        $result['BASKET_ID'] = $currentId;
        $result['BASKET_OFFER_ID'] = $offerId;
        $result['CODE'] = 'SUCCESS';

        $result['SUCCESS'] = $resultObj->isSuccess();
        //$result['ERRORS'] = $resultObj->getErrorMessages();
	...
	protected function getBasketTotal()
	{
        ...
        $result['allSum_BASE'] = PriceMaths::roundPrecision($basketBasePrice);
        $result['allSum_BASE_FORMATED'] = CCurrencyLang::CurrencyFormat($result['allSum_BASE'], $siteCurrency, true);
	...
	protected function selectOfferById($iblockId, $productId, $currentOfferId, $newOfferId, $propertyCodes)
	{
        ...
        if (empty($offers[$productId][$newOfferId])) {
            return null;
        }

        return $offers[$productId][$newOfferId];
	}
    ...
    // its очистить всю корзину
    protected function removeallAction()
    {
        global $APPLICATION;
        CSaleBasket::DeleteAll(CSaleBasket::GetBasketUserID());
        ////
        //from recalculateAjax
        //////
