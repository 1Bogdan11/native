...
class JournalPersonalOrderListComponent extends CBitrixComponent
{
	...
	protected function prepareFilter()
	{
        ...
        if ($this->arParams['SHOW_TYPE'] === 'HISTORY') {
            $arFilter[] = array(
                'LOGIC' => 'OR',
        ...
        for ($i = 0; $i < 10; $i++) {
            $date->modify('+ 1 year');
            $allowedYears[] = $date->format('Y');
        }

        if (in_array($this->arParams['SHOW_YEAR'], $allowedYears)) {
            $arFilter[] = [
                '>=DATE_INSERT' => date(
                    $DB->DateFormatToPHP(
                        $this->dateFormat
                    ),
                    mktime(0, 0, 0, 1, 1, $this->arParams['SHOW_YEAR'])
                ),
                '<=DATE_INSERT' => date(
                    $DB->DateFormatToPHP(
                        $this->dateFormat
                    ),
                    mktime(23, 59, 59, 12, 31, $this->arParams['SHOW_YEAR'])
                ),
            ];
        }
