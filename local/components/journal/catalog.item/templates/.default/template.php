...
?>
<li class="catalog-section__item" id="<?=$arParams['AREA_ID']?>">
            ...
            if (!empty($arResult['OFFERS'])) {
                foreach ($arResult['OFFERS'] as $arOffer) {
                    foreach ($arOffer['PROPERTIES'] as $arProperty) {
                        ...
                        $colorFilter = $arParams['CURRENT_FILTER']['OFFERS']["PROPERTY_{$arProperty['ID']}"]
                            ?? $arParams['CURRENT_FILTER']['OFFERS']["=PROPERTY_{$arProperty['ID']}"];
                        $currentValue = strval(is_array($arProperty['VALUE']) ? reset($arProperty['VALUE']) : $arProperty['VALUE']);
                        if (empty($colorFilter) || !in_array($currentValue, $colorFilter)) {
                            continue;
                        }
                        $offerPictures = Template::selectPictures($arOffer);
